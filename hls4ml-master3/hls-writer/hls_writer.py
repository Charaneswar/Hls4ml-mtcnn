...
def hls_writer(layer_list, yamlConfig):
    ...
    if not is_conv2d:
        for i in range(1, len(layer_list) + 1):
            if layer_list[i - 1]['class_name'] == 'Dense' or layer_list[i - 1]['class_name'] == 'BinaryDense' or \
                    layer_list[i - 1]['class_name'] == 'TernaryDense':
                is_dense = True
                break
    ...
    for line in f.readlines():
        ...
        # Add layers
            ...
            for i in range(1, len(layer_list) + 1):
                ...
                if i == 1 and (layer_list[i - 1]['class_name'] == 'Dense' or layer_list[i - 1][
                    'class_name'] == 'BinaryDense' or layer_list[i - 1]['class_name'] == 'TernaryDense' or (
                                       layer_list[i - 1]['class_name'] == 'BatchNormalization' and is_dense)):
                    input_type = 'input_t'
                    input_object = 'data'
                    n_in = 'N_INPUTS'
                ...
                elif layer_list[i - 1]['class_name'] == 'Dense' or layer_list[i - 1]['class_name'] == 'BinaryDense' or \
                        layer_list[i - 1]['class_name'] == 'TernaryDense' or layer_list[i - 1][
                    'class_name'] in activation_layers:
                    input_type = 'layer{}_t'.format(i - 1)
                    input_object = 'layer{}_out'.format(i - 1)
                ...
                if i == len(layer_list) and (layer_list[i - 1]['class_name'] == 'Dense' or layer_list[i - 1][
                    'class_name'] == 'BinaryDense' or layer_list[i - 1]['class_name'] == 'TernaryDense'):
                    ...
                    out_width = 'OUT_WIDTH_{}'.format(i)
                    n_filt = 'N_FILT_{}'.format(i)
                elif (i == len(layer_list) - 1 and is_dense and layer_list[i - 1][
                    'class_name'] == 'BatchNormalization' and layer_list[i]['class_name'] in activation_layers):
                    output_type = 'result_t'
                    output_object = 'layer{}_out'.format(i)
                    n_out = 'N_OUTPUTS'
                elif layer_list[i - 1]['class_name'] == 'Dense' or layer_list[i - 1]['class_name'] == 'BinaryDense' or \
                        layer_list[i - 1]['class_name'] == 'TernaryDense' or (
                        layer_list[i - 1]['class_name'] == 'BatchNormalization' and is_dense) or (
                        layer_list[i - 1]['class_name'] in activation_layers and is_dense):
                    output_type = 'layer{}_t'.format(i)
                    output_object = 'layer{}_out'.format(i)
                    ...
                    n_filt = 'N_FILT_{}'.format(i)
                elif layer_list[i - 1]['class_name'] == 'Conv2D' or (
                        is_conv2d and layer_list[i - 1]['class_name'] == 'BatchNormalization'):
                    output_type = 'layer{}_t'.format(i)
                    output_object = 'layer{}_out'.format(i)
                    out_height = 'OUT_HEIGHT_{}'.format(i)
                    out_width = 'OUT_WIDTH_{}'.format(i)
                ...
                if (i != len(layer_list)):
                    if layer_list[i - 1]['class_name'] == 'Dense' or layer_list[i - 1]['class_name'] == 'BinaryDense' or \
                        ...
                        newline += '    {} layer{}_out[{}*{}*{}];\n'.format(output_type, i, out_height, out_width,
                                                                            n_filt)
                        ...
                        if i != 1:
                            ...
                            newline += '    {} layer{}_out[{}*{}*{}];\n'.format(output_type, i, in_height, in_width,
                                                                                n_filt)
                    newline += '    #pragma HLS ARRAY_PARTITION variable=layer{}_out complete dim=0\n'.format(i)
                ...
                if layer_list[i - 1]['class_name'] == 'Dense' or layer_list[i - 1]['class_name'] == 'BinaryDense' or \
                        layer_list[i - 1]['class_name'] == 'TernaryDense':
                    newline += '    {} logits{}[{}];\n'.format(output_type, i, n_out)
                    newline += '    #pragma HLS ARRAY_PARTITION variable=logits{} complete dim=0\n'.format(i)

                    if layer_list[i - 1]['n_part'] == 1:
                        # Use one layer if there's only 1 partition
                        ...
                        for i_part in range(1, layer_list[i - 1]['n_part'] - 1):
                            ...
                            sublayerline += '    #pragma HLS ARRAY_PARTITION variable=logits{}_0to{} complete dim=0\n'.format(
                                i, i_part)
                        # compute sublayer outputs
                        for i_part in range(0, layer_list[i - 1]['n_part']):
                            sublayerline += '    nnet::compute_layer<{}, {}, config{}_{}>({}, logits{}_{}, w{}_{}, b{}_{});\n'.format(
                                input_type, output_type, i, i_part, input_object, i, i_part, i, i_part, i, i_part)
                        ...
                        for i_part in range(0, layer_list[i - 1]['n_part'] - 1):
                            ...
                            if layer_list[i - 1]['n_part'] == 2:
                                sublayerline += '    nnet::merge<{}, {}, {}>(logits{}_{}, logits{}_{}, logits{});\n'.format(
                                    output_type, n_mergeout, n_subout, i, i_part, i, i_part + 1, i)
                            elif i_part == 0:
                                sublayerline += '    nnet::merge<{}, {}, {}>(logits{}_{}, logits{}_{}, logits{}_0to{});\n'.format(
                                    output_type, n_mergeout, n_subout, i, i_part, i, i_part + 1, i, i_part + 1)
                    ...
                    if i > 1 and layer_list[i - 2]['class_name'] == 'Conv1D':
                        ...
                        newline += '    #pragma HLS ARRAY_PARTITION variable=conv_layer{}_in complete dim=0\n'.format(i)
                        newline += '    nnet::unflatten<{}, {}, {}>({}, conv_layer{}_in);\n'.format(input_type, y_in,
                                                                                                    n_chan,
                                                                                                    input_object, i)
                        newline += '    {} conv_layer{}_out[{}][{}];\n'.format(output_type, i, y_out, n_filt)
                        newline += '    #pragma HLS ARRAY_PARTITION variable=conv_layer{}_out complete dim=0\n'.format(
                            i)
                        newline += '    nnet::conv_1d<{}, {}, config{}>(conv_layer{}_in, conv_layer{}_out, w{}, b{});\n'.format(
                            input_type, output_type, i, i, i, i, i, i)
                    else:
                        newline += '    {} conv_layer{}_out[{}][{}];\n'.format(output_type, i, y_out, n_filt)
                        newline += '    #pragma HLS ARRAY_PARTITION variable=conv_layer{}_out complete dim=0\n'.format(
                            i)
                    ...
                    newline += '    {} logits{}[{}*{}];\n'.format(output_type, i, y_out, n_filt)
                    newline += '    #pragma HLS ARRAY_PARTITION variable=logits{} complete dim=0\n'.format(i)
                    newline += '    nnet::flatten<{}, {}, {}>(conv_layer{}_out, logits{});\n'.format(input_type, y_out,
                                                                                                     n_filt, i, i)
                elif layer_list[i - 1]['class_name'] == 'Conv2D':
                    if i > 1 and (layer_list[i - 2]['class_name'] == 'Conv2D' or layer_list[i - 2][
                        'class_name'] == 'BatchNormalization'):
                        newline += '    {} conv2d_layer{}_in[{}][{}][{}];\n'.format(input_type, i, in_height, in_width,
                                                                                    n_chan)
                        newline += '    #pragma HLS ARRAY_PARTITION variable=conv2d_layer{}_in complete dim=0\n'.format(
                            i)
                        newline += '    nnet::unflatten<{}, {}, {}, {}>({}, conv2d_layer{}_in);\n'.format(input_type,
                                                                                                          ...
                                                                                                          input_object,
                                                                                                          i)
                        newline += '    {} conv2d_layer{}_out[{}][{}][{}];\n'.format(output_type, i, out_height,
                                                                                     out_width, n_filt)
                        newline += '    #pragma HLS ARRAY_PARTITION variable=conv2d_layer{}_out complete dim=0\n'.format(
                            i)
                    ...
                    newline += '    nnet::normalize<{}, {}, config{}>({}, {}, scale{}, beta{}, mean{});\n'.format(
                        input_type, output_type, i, input_object, output_object, i, i, i)
                elif i == 1 and layer_list[i - 1]['class_name'] == 'BatchNormalization' and is_conv2d:
                    newline += '    {} logits{}[{}*{}*{}];\n'.format(output_type, i, in_height, in_width, n_filt)
                    newline += '    #pragma HLS ARRAY_PARTITION variable=logits{} complete dim=0\n'.format(i)
                    newline += '    nnet::flatten<{}, {}, {}, {}>({}, logits{});\n'.format(input_type, in_height,
                                                                                           in_width, n_filt,
                                                                                           input_object, i)
                    newline += '    nnet::normalize<{}, {}, config{}>(logits{}, {}, scale{}, beta{}, mean{});\n'.format(
                        output_type, output_type, i, i, output_object, i, i, i)
                elif layer_list[i - 1]['class_name'] == 'BatchNormalization' and is_conv2d:
                    newline += '    nnet::normalize<{}, {}, config{}>({}, {}, scale{}, beta{}, mean{});\n'.format(
                        input_type, output_type, i, input_object, output_object, i, i, i)
                elif 'Pooling' in layer_list[i - 1]['class_name']:
                    ...
                    if d == 1:
                        newline += '    nnet::pooling1d<{}, config{}>({}, {});\n'.format(input_type, i, input_object,
                                                                                         output_object)
                    elif d == 2:
                        ...
                        if unflatten:
                            ...
                            newline += '    #pragma HLS ARRAY_PARTITION variable=pool2d_layer{}_in complete dim=0\n'.format(
                                i)
                            newline += '    nnet::unflatten<{}, {}, {}, {}>({}, pool2d_layer{}_in);\n'.format(
                                input_type, in_height, in_width, n_filt, input_object, i)
                            outshape = ''.join('[{0}]'.format(dim) for dim in [out_height, out_width, n_filt])
                            newline += '    {} pool2d_layer{}_out{};\n'.format(input_type, i, outshape)
                            newline += '    #pragma HLS ARRAY_PARTITION variable=pool2d_layer{}_out complete dim=0\n'.format(
                                i)
                        ...
                        newline += '    nnet::flatten<{}, {}, {}, {}>(pool2d_layer{}_out, layer{}_out);\n'.format(
                            input_type, out_height, out_width, n_filt, i, i)
    ...
    for line in f.readlines():
        ...
        if '//hls-fpga-machine-learning insert numbers' in line:
            ...
            for i in range(1, len(layer_list) + 1):
                ...
                elif i == len(layer_list) and (layer_list[i - 1]['class_name'] == 'Dense' or layer_list[i - 1][
                    'class_name'] == 'BinaryDense' or layer_list[i - 1]['class_name'] == 'TernaryDense'):
            ...
            for i in range(1, len(layer_list) + 1):
                if i == 1 and (layer_list[i - 1]['class_name'] == 'Dense' or layer_list[i - 1][
                    'class_name'] == 'BinaryDense' or layer_list[i - 1]['class_name'] == 'TernaryDense' or
                               layer_list[i - 1]['class_name'] == 'BatchNormalization'):
                ...
                elif i == len(layer_list) and layer_list[i - 1]['class_name'] == 'Dense' and layer_list[i - 2][
                    'class_name'] == 'Conv1D':
                    ...
                    'class_name'] == 'BinaryDense' or layer_list[i - 1]['class_name'] == 'TernaryDense' or (
                                                       is_dense and layer_list[i - 1][
                                                   'class_name'] in activation_layers) or (
                                                       is_dense and layer_list[i - 1][
                                                   'class_name'] == 'BatchNormalization')):
                    layer_in_name = "N_LAYER_{}".format(i - 1)
                    layer_out_name = "N_OUTPUTS"
                        ...
                        layer_list[i - 1]['class_name'] == 'TernaryDense' or (
                        is_dense and layer_list[i - 1]['class_name'] in activation_layers):
                    layer_in_name = "N_LAYER_{}".format(i - 1)
                    layer_out_name = "N_LAYER_{}".format(i)
                elif layer_list[i - 1]['class_name'] == 'Conv1D':
                    ...
                    'class_name'] == 'Conv2D':  # or (is_conv2d and layer_list[i-1]['class_name']=='BatchNormalization'):
                ...
                if layer_list[i - 1]['class_name'] == 'Dense' or layer_list[i - 1]['class_name'] == 'BinaryDense' or \
                    ...
                    if layer_list[i - 1]['n_part'] == 1:
                        ...
                        for i_part in range(0, layer_list[i - 1]['n_part']):
                            newline += dense_sub_config_template.format(index=str(i),
                                                                        ...
                                                                        iotype='io_parallel',
                                                                        reuse=yamlConfig["ReuseFactor"],
                                                                        nzeros=layer_list[i - 1]['weights_n_subzeros'][
                                                                            i_part])
                    ...
                    newline += activ_config_template.format(type=layer_list[i - 1]['activation'],
                                                            index=str(i),
                                                            n_in='{}*{}*{}'.format(layer_out_height_name,
                                                                                   layer_out_width_name,
                                                                                   layer_n_filt_name),
                                                            iotype='io_parallel')
                elif 'Pooling' in layer_list[i - 1]['class_name']:
                    info = layer_list[i - 1]['class_name'].split('Pooling')
    ...
    shutil.copyfile(yamlConfig['OutputPredictions'],
                    '{}/tb_data/tb_output_predictions.dat'.format(yamlConfig['OutputDir']))
    ...
    for line in f.readlines():
        ...
        if 'myproject' in line:
            newline = line.replace('myproject', yamlConfig['ProjectName'])
        elif '//hls-fpga-machine-learning insert data' in line and (
                layer_list[0]['class_name'] == 'Dense' or layer_list[0]['class_name'] == 'BinaryDense' or layer_list[0][
            'class_name'] == 'TernaryDense' or (is_dense and layer_list[0]['class_name'] == 'BatchNormalization')):
            newline += '      input_t  data_str[N_INPUTS] = {'
            newline += ','.join('in[{}]'.format(i) for i in range(0, layer_list[0]['n_in']))
            newline += '};\n'
            ...
            newline += ','.join('in[{}]'.format(i) for i in range(0, layer_list[0]['y_in'] * layer_list[0]['n_chan']))
            newline += '};\n'
            ...
            newline += ','.join('in[{}]'.format(i) for i in range(0, layer_list[0]['in_height'] * layer_list[0][
                'in_width'] * layer_list[0]['n_chan']))
            newline += '};\n'
        elif '//hls-fpga-machine-learning insert data' in line and is_conv2d and layer_list[0][
            'class_name'] == 'BatchNormalization':
            newline += '      input_t  data_str[IN_HEIGHT_1][IN_WIDTH_1][N_FILT_1] = {'
            newline += ','.join('in[{}]'.format(i) for i in range(0, layer_list[0]['in_height'] * layer_list[0][
                'in_width'] * layer_list[0]['n_filt']))
            newline += '};\n'
        else:
            newline = line
        fout.write(newline)
